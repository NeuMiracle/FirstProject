...
f2 = open("features/Basefeature_user.csv")
itemdata = csv.reader(f2)
itemdata.next()
for row in itemdata:
    dict_feature2[row[0]] = float(row[3])
f2.close()
...
def GetData(csvpath):
    ...
    for key in dict_cart:
        ...
        if not dict_feature1.has_key(user_id):
            continue
        if not dict_feature2.has_key(user_id):
            continue
        if dict_abnormal.has_key(user_id):
            continue
        dict_data[key] = [dict_feature1[user_id],dict_feature2[user_id]]
    return dict_data
...
def ProduceTrainset():
    ...
    write1.writerow(["user_id", "item_id", "user_ratio", "user_cartnum"])


    w_f2 = open("data/traindata/trainset_n.csv", 'wb')
    write2 = csv.writer(w_f2)
    write2.writerow(["user_id", "item_id", "user_ratio", "user_cartnum"])
