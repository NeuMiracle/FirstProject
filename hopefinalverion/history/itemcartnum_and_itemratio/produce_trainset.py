...
dict_feature2 = {}
dict_abnormal = {}
f1 = open("features/Basefeature_item.csv")
userdata = csv.reader(f1)
userdata.next()
for row in userdata:
    dict_feature1[row[0]] = float(row[3])
f1.close()
...
def GetData(csvpath):
    ...
    for key in dict_cart:
        if dict_buy.has_key(key):
            continue
        user_id, item_id = key
        if not dict_feature1.has_key(item_id):
            continue
        if not dict_feature2.has_key(item_id):
            continue
        if dict_abnormal.has_key(user_id):
            continue
        dict_data[key] = [dict_feature1[item_id],dict_feature2[item_id]]
    return dict_data
...
def ProduceTrainset():
    ...
    write1.writerow(["user_id", "item_id", "item_cartnum", "item_ratio"])


    w_f2 = open("data/traindata/trainset_n.csv", 'wb')
    write2 = csv.writer(w_f2)
    write2.writerow(["user_id", "item_id", "item_cartnum", "item_ratio"])
